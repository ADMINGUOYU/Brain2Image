# EEG-CLIP alignment model
# Aligns EEG encoder outputs directly to ViT-H-14 CLIP CLS embeddings (1024-dim).
...
class EEG_CLIP_Align(nn.Module):
    """
    EEG_CLIP_Align

    Pipeline:
        eeg_input -> eeg_encoder (backbone) -> projection head -> F.normalize -> (B, 1024)

    Supports the same backbone × pooling combinations as EEG_fMRI_Align, but
    targets 1024-dim (ViT-H-14 CLIP CLS) instead of 4096-dim fMRI.

    For ATMS: proj_dim=1024 already produces (B, 1024) natively; no output_mlp needed.
    For CBraMod: projection heads target embedding_dim=1024.
    """
    ...
    def __init__(self, param: typing.Dict):
        ...
        """
        super(EEG_CLIP_Align, self).__init__()
        ...
        if self.encoder_type == 'CBraMod':
            ...
            out_dim = enc_cfg.get('embedding_dim', 1024)
            ...
            if self.pooling_type == 'flatten':
                ...
                self.multitoken_vit = MultiTokenViT(
                    ...
                    output_dim=out_dim  # Use embedding_dim from config (1024 for CLIP)
                )
            else:
                raise ValueError(f"Unsupported pooling type: {self.pooling_type}")

        elif self.encoder_type == 'ATMS':
            # proj_dim=1024 already gives (B, 1024); out_mlp_dim not needed for CLIP alignment
    ...
    def forward(self, eeg_input: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            eeg_input: (B, 63, 1, T)
        Returns:
            eeg_embeds: (B, 1024), unit-normalized
        """
    ...
    def calc_alignment_loss(self,
        ...
        """
        MSE + InfoNCE alignment loss.

        Args:
            aligned_embeds: (B, 1024) — unit-normalized EEG embeddings (from forward())
            target_embeds:  (B, 1024) — CLIP-H CLS embeddings
        Returns:
    ...
    def get_metrics_for_alignment(self,
        ...
        retrieval_acc = (top1 == labels).float().mean().item()
...
if __name__ == "__main__":
    ...
    param_cbramod = {
        ...
        'Loss': {
            'mse_scale': 1.0, 'infonce_scale': 0.2,
            'temperature': 0.1, 'normalize_clip': True,
        },
    }
    model = EEG_CLIP_Align(param_cbramod)
    dummy_eeg = torch.randn(batch_size, 63, 1, 200)
    out = model(dummy_eeg)
    print("CBraMod output shape:", out.shape)  # (4, 1024)
    ...
    model_atms = EEG_CLIP_Align(param_atms)
    dummy_eeg_atms = torch.randn(batch_size, 63, 1, 250)
    out_atms = model_atms(dummy_eeg_atms)
    print("ATMS output shape:", out_atms.shape)  # (4, 1024)

    # Test loss
    clip_target = torch.randn(batch_size, 1024)
    total, mse, infonce = model_atms.calc_alignment_loss(out_atms, clip_target)
    print(f"Loss — total: {total:.4f}, MSE: {mse:.4f}, InfoNCE: {infonce:.4f}")
