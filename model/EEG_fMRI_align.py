...
# We use the embeddings generated by eeg_encoder 
# (CBraMod back-bone or ATMS)
...
class EEG_fMRI_Align(nn.Module):
    ...
    def __init__(self, param: typing.Dict):
        ...
        Args:
            ...
            - param['EEG_Encoder'] Dict: Configuration for the EEG encoder (CBraMod_EEG_Encoder)
                - param['EEG_Encoder']['encoder_type']: Type of EEG encoder to use ('CBraMod' or 'ATMS')
        ...
        # EEG encoder
        if param['EEG_Encoder'].get('encoder_type', 'CBraMod') == 'CBraMod':
            self.eeg_encoder = CBraMod_EEG_Encoder(param['EEG_Encoder'])
        elif param['EEG_Encoder']['encoder_type'] == 'ATMS':
            self.eeg_encoder = ATMS_EEG_Encoder(param['EEG_Encoder'])
        else:
            raise ValueError(f"Unsupported EEG encoder type: {param['EEG_Encoder']['encoder_type']}")
    ...
    def load_model(self, path: str, device: torch.device):
        ...
        # Check if the encoder type matches
        ...
        if saved_encoder_type != current_encoder_type:
            raise ValueError(f"Encoder type mismatch: saved model has {saved_encoder_type}, but current model has {current_encoder_type}. Please initialize the model with the same encoder type as the saved model.")
        # Load state dicts and parameters
